					...
					if (agRes.Status == PromptStatus.OK)
					{
						...
						if (phiAgRes.Status == PromptStatus.OK)
						{
							...
							using (Transaction trans = AutoCAD.curDb.TransactionManager.StartTransaction())
							{
								...
								// Get the NOD in the database
								DBDictionary nod =
									(DBDictionary) trans.GetObject(AutoCAD.curDb.NamedObjectsDictionaryId,
										OpenMode.ForWrite);
			...
			public static void SetSteelParameters()
			{
				...
				using (Transaction trans = AutoCAD.curDb.TransactionManager.StartTransaction())
				{
					...
					if (fyRes.Status == PromptStatus.OK)
					{
						...
						// Get the NOD in the database
						DBDictionary nod = (DBDictionary) trans.GetObject(AutoCAD.curDb.NamedObjectsDictionaryId,
							OpenMode.ForWrite);
