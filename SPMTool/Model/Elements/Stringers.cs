...
using UnitsNet;
using Stringers = SPMTool.Model.Elements.Stringers;
...
namespace SPMTool.Model.Elements
{
	...
	public static class Stringers
	{
        ...
        public static void Add(Point3d startPoint, Point3d endPoint) => Add(new Line(startPoint, endPoint));
        ...
        /// <param name="endPoint">The end <see cref="Point3d"/>.</param>
        /// <param name="stringerCollection">The collection containing all the stringer geometries in the drawing.</param>
		public static void Add(Point3d startPoint, Point3d endPoint, ref IEnumerable<StringerGeometry> stringerCollection) => Add(new Line(startPoint, endPoint), ref stringerCollection);
		...
		public static void DivideStringer()
		{
            ...
            using (var trans = DataBase.StartTransaction())
            {
                foreach (Line str in strs)
                {
                    ...
                    using (var ndsToErase = new ObjectIdCollection((from DBPoint intNd in intNds where intNd.Position.Approx(midPt) select intNd.ObjectId).ToArray()))
		...
		public static ObjectIdCollection UpdateStringers(bool updateNodes = true)
		{
            ...
            using (var trans = DataBase.StartTransaction())
            {
	            ...
	            foreach (ObjectId strObj in strObjs)
					using (var str = (Line)trans.GetObject(strObj, OpenMode.ForWrite))
		            {
		...
		public static IEnumerable<StringerGeometry> StringerGeometries()
		{
			...
			return
				from Line line in strs select new StringerGeometry(line.StartPoint, line.EndPoint, 0, 0);
		}
		...
		private static StringerGeometry? GetStringerGeometry(Units units)
		{
			...
			if (savedGeo != null)
			{
				...
				// Get string result
				var res = UserInput.SelectKeyword($"Choose a geometry option ({dimAbrev} x {dimAbrev}) or add a new one:", options.ToArray(), out var index, options[0]);
			...
			// New reinforcement
			var def = 100.ConvertFromMillimeter(units.Geometry);
