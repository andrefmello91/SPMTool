...
namespace SPMTool
{
    ...
    {
        // List of stringer parameters
        public List<Tuple<int, int, double>> StringerParams(ObjectIdCollection stringers)
        {
            // Initialize a list
            var strPrms = new List<Tuple<int, int, double>>(stringers.Count);

            // Start a transaction
            using (Transaction trans = AutoCAD.curDb.TransactionManager.StartTransaction())
            {
                // Get the parameters of each stringer and add to the list
        ...
        public class Linear
        {
            ...
            public List<Tuple<int, int[], Matrix<double>, Matrix<double>>> StringersStifness(ObjectIdCollection stringers, List<Point3d> nodeList, double Ec, Matrix<double> Kg)
            {
            ...
            public List<Tuple<int, int[], Matrix<double>, Matrix<double>>> PanelsStiffness(ObjectIdCollection panels, List<Point3d> nodeList, double Gc, Matrix<double> Kg)
            {
                // Initialize a tuple list to store the matrices of stringers
